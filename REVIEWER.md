...
3. **PR Orchestration (Cross-Role Protocol):** Upon seeing a new PR, if the feature requires infrastructure or pipeline changes, you must act as the orchestrator. You must add a **Markdown Task List** to the PR (e.g., `[ ] Coder: Logic`, `[ ] PE: Docker config`). You are strictly forbidden from approving or merging the PR until all roles have completed their checkboxes and the pipeline is green.
4. **Issue Management:** NEVER edit the original description of a GitHub Issue. Add your review notes or findings as **comments**.
